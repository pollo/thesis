...
An adaptation of the model was additionally realized in order to simulate ski lessons. In these simulations agents have strongly correlated trajectories: the first agent simulates the instructor and decide the trajectory autonomously, the following agents are forced to emulate the instructor trajectory.

The implementation was realized in C++ using the GRASS GIS C-API for the spatial computations. GRASS GIS is an Open Source Geographic Information System (GIS) software used for geospatial data management and analysis.

The mobile application SkiLogger, developed inside the SicurSkiWeb project, was used to collect data of real skiers. SkiLogger can be used by skiers to track their trajectories using GPS sensors, now usually available also on common smartphones. The application was released for beta testing in the February 2013, allowing to track a total of 171 Km of descents on ski slopes by 18 distinct skiers. The data collected were not enough to do a robust calibration of the parameters and to validate the model. A comparison between real data and simulated data was performed in terms of speed values. The average error for the tracks considered ranges between 4.28 m/s and 6.44 m/s. This result is affected by the stops made by the skiers tracked. Introducing a new experimental stop parameter the average error made for a specific track was reduced from 5.56 m/s to 2.52 m/s.

Figure \ref{traj} shows simulated and recorded trajectories on the ski slope ``Uno di Tognola'' of San Martino di Castrozza in Trentino (Italy).

\begin{figure}[!h]
        \centering
        \begin{subfigure}[b]{0.5\textwidth}
                \centering
                \includegraphics[width=\textwidth]{images/simulated_trajectories.eps}
                \caption{Simulated trajectories}\label{traj_sim}
        \end{subfigure}%
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc.
          %(or a blank line to force the subfigure onto a new line)
        \begin{subfigure}[b]{0.5\textwidth}
                \centering
                \includegraphics[width=\textwidth]{images/real_trajectories.eps}
                \caption{Recorded trajectories}\label{traj_real}
        \end{subfigure}
